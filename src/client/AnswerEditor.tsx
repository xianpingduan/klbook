...
import type { AnswerEdit, OriginalPage, Question, QuestionPart } from '../shared/collection.ts';
...
export function AnswerEditor({ api, question, cache, grant, active, onBack, onSaved, onAccessError }: {
  ...
  const pending = useRef<AnswerEdit | null>(null);
  ...
  const leave = useEditorLeave({ dirty: JSON.stringify(parts) !== baseline.current, busy: working, canSave: active, title: '纸质答案还有未保存的修改', error,
    ...
    discard: () => { setParts(current.current.answerParts); baseline.current = JSON.stringify(current.current.answerParts); pending.current = null; } });
  async function send(input: AnswerEdit) {
    pending.current = input;
    try {
      const saved = await api.saveAnswers(question.id, input, grant);
      if (saved.revision !== input.expectedRevision + 1) throw new ApiError(409, '这道题已在其他页面更新，请返回列表重新打开后核对');
  ...
  async function save() {
    ...
    try {
      const desired = content(parts);
      if (pending.current) await send(pending.current);
      if (JSON.stringify(desired) !== JSON.stringify(content(current.current.answerParts))) await send({ operationId: crypto.randomUUID(), expectedRevision: current.current.revision, parts: desired });
      await append.committed({ parts: current.current.answerParts });
      if (!mounted.current) return false;
      baseline.current = JSON.stringify(parts); onSaved(current.current); return true;
    } catch (failure) {
      if (!mounted.current) return false;
  ...
  return <section className="card collection-card answer-editor">
    ...
    <fieldset disabled={working || !active}>
      ...
      <div className="answer-sources"><h2>选择答案材料</h2>
        ...
        {append.capture ? <><p>{append.capture.name} · {append.appended ? '保存答案后完成关联。' : '本机保留的答案图片，可继续。'}</p>{!append.appended && <><button className="quiet" onClick={append.retry}>继续追加 {append.capture.name}</button><button className="quiet" onClick={append.cancel}>取消追加</button></>}</> : <div className="answer-capture"><CaptureInput label="拍摄答案" camera busy={working || !active || parts.length >= 50} onChoose={append.choose} onCancel={() => {}} /><CaptureInput label="选择答案图片" multiple={false} busy={working || !active || parts.length >= 50} onChoose={append.choose} onCancel={() => {}} /></div>}
