...
export class CaptureCache {
  ...
  async saveBatch(batch: CaptureBatch) {
    const header = new TextEncoder().encode(JSON.stringify({ version: 2, total: batch.total, uploaded: batch.uploaded, cancelled: batch.cancelled,
      items: batch.items.map(item => ({ operationId: item.operationId, name: item.name, type: item.file.type, size: item.file.size, studyStage: item.studyStage })) }));
  ...
  async readBatch(): Promise<CaptureBatch | undefined> {
    ...
    try {
      ...
      let offset = length + 4;
      const items: PendingCapture[] = header.items.map((item: { operationId: string; name: string; type: string; size: number; studyStage?: unknown }) => {
        ...
        const studyStage = item.studyStage ?? emptyStage;
        if (!validStage(studyStage)) throw new Error('Invalid study stage');
        return { operationId: item.operationId, name: item.name, file, studyStage };
      });
