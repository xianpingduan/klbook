...
export function SourceManager({ api, grant, active = true, onAccessError }: { api: FamilyApi; grant?: string; active?: boolean; onAccessError(error: ApiError): Promise<void> }) {
  ...
  const leave = useEditorLeave({
    dirty, busy: saving, canSave: !loading && !!grant && active, title: '来源还有未保存的修改', error,
    description: <p>{grant ? '可以先保存来源再离开。放弃本次修改不会删除已有来源。' : '管理验证已到期，选择继续编辑并重新验证后才能保存。'}</p>,
    save: saveForm, discard: clearForm,
  });
  ...
  return <section className="source-manager" aria-label="来源管理列表">
    ...
    <form onSubmit={submit} className="source-form"><fieldset disabled={busy || !grant || !active}>
      ...
      <div className="source-actions"><button type="submit">{editing ? '保存来源' : '添加来源'}</button>{(editing || dirty) && <button type="button" className="quiet" onClick={() => leave.requestLeave(clearForm)}>取消修改</button>}</div>
    </fieldset></form>
