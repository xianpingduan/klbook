...
test('手机尺寸学习端追加图片，未确认的新范围阻止保存离开，完成后刷新仍可查看整道题', async ({ page, request }) => {
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: '打开错题', exact: true })).toBeEnabled();
    const storageTab = await page.context().newPage();
    await storageTab.goto(f.url);
    // Block actual app-created storage without depending on database names, versions or store layout.
    await storageTab.evaluate(async () => {
      const databases = await indexedDB.databases();
      if (!databases.length) throw new Error('The app has not initialized its device storage');
      let released = false;
      ...
      await Promise.all(databases.map(database => new Promise<void>((resolve, reject) => {
        if (!database.name) { reject(new Error('Device database has no name')); return; }
        ...
        opening.onsuccess = () => {
          ...
          const keepActive = () => { const read = transaction.objectStore(stores[0]!).get('test-lock'); read.onsuccess = () => { resolve(); if (!released) keepActive(); }; };
