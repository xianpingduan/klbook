...
- 写入顺序：解码验证 → 独立临时目录 → 两个文件写完并各自同步磁盘 → 同卷重命名 → 短 SQLite 事务提交原始页、草稿及操作标识。转换与文件 I/O 不占数据库事务。
...
上传前通过平台存储接口将图片、原文件名与操作 UUID 写为一个 Blob，按家庭资料库和账号隔离。长度头、JSON 元信息和图片字节在同一次 IndexedDB 事务中保存，避免图片与重试标识分离。上传失败可重试，页面刷新后可选择继续上传；确认服务端保存后删除本设备暂存。
