...
学习端入口为 `/learn`，保留首页、收集、我的三个 Tab；家长管理使用独立 `/admin` 入口，验证身份后进入真实管理概览，再通过侧栏管理资料、来源及设备。两端共用原家庭资料库。[入口设计与兼容边界](docs/ui-entry-implementation.md)及[验证结果](docs/verification/issue-29.md)单独记录。
...
后端只监听本机 `127.0.0.1`；手机和平板通过 Caddy HTTPS 网关访问。配置、证书信任、防火墙和真机试用步骤见[局域网采集](docs/lan-capture.md)。Windows 开机自启仍由 #22 实施；网页入口不等同于手机安装包。

客户端可在登录前或“我的 → 连接设置”填写服务地址，先测试兼容性，再确认使用并登录目标资料库。暂不可达的地址可以单独保存待连接，原连接保留；不同网页来源需在目标电脑的允许来源中配置。详见[服务地址设置](docs/service-address.md)及[验证记录](docs/verification/issue-11.md)。

## 收集一道错题

登录后进入底部“收集”，点击“收集一道错题”，选择一张 JPEG、PNG 或静态 WebP 图片（最多 15 MiB、4000 万像素）。拖动框住题目，或点击“选择整页”，选好学科即可保存。来源、页码、题号、备注可留空，也可日后在详情中补充。

可以拍照、从相册多选或电脑上传。每批最多 10 张、合计 75 MB，逐张整理；待上传材料可单张取消，刷新后可以继续剩余图片。HEIC 原件需先在相册导出为 JPEG。详见[局域网与批量采集说明](docs/lan-capture.md)。
...
“来源”使用下拉选择，也可以留空。在“家长管理 → 来源管理”中，家长可新增、改名、停用和重新启用来源。改名统一更新关联题目的显示，停用保留旧题归属；之前手填的来源升级时自动加入列表。详见[来源管理](docs/source-management.md)。

## 家长管理与恢复

- 每台设备用家长账号、密码登录后，只获得日常会话，默认持续 30 天；退出、撤销、恢复账号会提前使它失效。
- 从“我的 → 家长管理”或直接打开 `/admin`，再次输入密码；管理授权只属于当前设备，5 分钟后失效。侧栏“设备与账号”可查看设备并撤销，顶部“结束管理”撤销管理授权并返回学习端。
