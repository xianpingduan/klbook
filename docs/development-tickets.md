...
2026-09-24，#11 已实现首次连接、登录前和“我的”中的服务地址设置，无凭据兼容性测试、待连接地址保存及明确登录所选资料库；其他浏览器设置不变。构建、32项API和三引擎141项浏览器完整回归通过，旧版原址升级及真实资料备份核验完成；代码审查基线仍等待用户确认，议题保持开放。Windows WebKit合成Cookie探针和Windows Edge家庭证书信任限制单独记录，未把未通过的HTTPS浏览器补测或真机记为通过。操作及证据见[服务地址](service-address.md)和[#11验证记录](verification/issue-11.md)。
