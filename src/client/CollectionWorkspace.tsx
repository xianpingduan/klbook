...
export function CollectionWorkspace({ api, home, platform, path, onExpired, onEditing, active = true, mode = 'workspace' }: {
  api: FamilyApi; home: Home; platform: ClientPlatform; path: PagePath; onExpired(): Promise<void>; onEditing(active: boolean): void; active?: boolean; mode?: 'home' | 'collect' | 'workspace';
}) {
  ...
  useLeaveGuard(busy && screen !== 'edit', () => setError('正在处理材料，请稍候再离开。当前材料仍保留。'));
  ...
  useEffect(() => {
    ...
    void Promise.all([api.subjects(), api.questions(listState), api.sources()]).then(([subjects, list, sources]) => {
      if (!current) return;
    ...
    return () => { current = false; };
  }, [api, listState, refresh, active, path, onExpired]);

  async function run(action: () => Promise<void>) {
    if (busy || cacheLoading) return;
  ...
  async function upload(capture: PendingCapture, currentBatch = batch!) {
    ...
    setBatch(next.items.length ? next : undefined); setSelected(question); setScreen(question.state === 'draft' ? 'edit' : 'detail'); setRefresh(value => value + 1);
  }
  ...
  function open(question: Question) {
    void run(async () => { const latest = await api.question(question.id); setSelected(latest); setOriginalOpen(false); setScreen(latest.state === 'draft' ? 'edit' : 'detail'); });
  }
  ...
  return <div className="collection-workspace">
    ...
    {screen === 'list' && <section className="card collection-card" aria-label={listState === 'draft' ? '已保存的草稿' : '已收集错题'}>
      <div className="section-heading"><div><h2>{mode === 'workspace' ? '我的学习材料' : listState === 'draft' ? '已保存的草稿' : '已收集'}</h2>{mode === 'collect' && <p className="hint">已保存在家庭电脑，可以换设备继续整理。</p>}</div>{mode === 'workspace' && <button disabled={busy} onClick={() => { setError(''); setScreen('upload'); }}>收集一道错题</button>}</div>
    ...
    {screen === 'detail' && selected && <section className="card collection-card">
      ...
      {pending && <div className="batch-next"><p>还有 {batch!.items.length} 张图片待处理 · 下一张：{pending.name}</p><button disabled={busy} onClick={() => void run(() => upload(pending))}>继续下一张</button><button className="quiet" disabled={busy} aria-label={`取消 ${pending.name}`} onClick={() => void run(() => cancel(pending))}>取消这张</button><button className="quiet" disabled={busy} onClick={back}>稍后继续</button><p className="hint">已上传 {batch!.uploaded} / {batch!.total} 张，已取消 {batch!.cancelled} 张</p></div>}
