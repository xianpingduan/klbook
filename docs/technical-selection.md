...
| 附件 | 后端管理的本地文件目录 | 原始页字节保留，预览/题目区为衍生文件；客户端只经鉴权 API 访问 |
...
- **仍按原任务决定**：OCR、语音转写、AI 供应商/模型分别在 #13、#15、#16 选定；每月 50 元服务预算保留。PWA 具体插件、图像解码库和正式证书安装配置在对应任务实测，不在此假装已经部署。
...
- 图片先写临时文件并完成校验/持久化，再以稳定附件 ID 关联数据库提交；失败可重试，未提交孤立文件可清理。原始文件不可就地覆盖，共享引用和回收站清理统一管理。
